...
const getDailyQuests = async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM daily_quest ORDER BY id ASC");
...
const getDailyQuestsById = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query("SELECT * FROM daily_quest WHERE id = $1", [id]);
...
const getReward = async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query("SELECT * FROM daily_quest WHERE reward_coins = $1", [reward_coins]);
        res.status(200).json(result.rows);
    } catch (error) {
...
const getDescription = async (req, res) => {
    try {
        const result = await pool.query("SELECT id, description FROM daily_quest ORDER BY id ASC");
        res.status(200).json(result.rows);
    } catch (error) {
...
const deleteDailyQuest = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            "DELETE FROM daily_quest WHERE id = $1 RETURNING *",
            [id]
        );
